...
function sanitizeSleepInput(s: string): string {
  ...
  if ((t.match(/\./g) || []).length > 1) {
    const parts = t.split('.');
    t = parts[0] + '.' + parts.slice(1).join('').slice(0, 2);
  }
  return t.slice(0, 6);
}
...
const InspectionFemaleStep3 = () => {
  ...
  const validation = useMemo(() => {
    ...
    const smokeOk = smokeNum !== null && smokeNum <= 80;

    const bingeNum = parseNonNegInt(bingeStr);
    const bingeOk = bingeNum !== null && bingeNum <= 366;

    const sleepNum = parseSleepHours(sleepStr);
    const sleepOk = sleepNum !== null;
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <input
                    ...
                    inputMode="numeric"
                    autoComplete="off"
                    placeholder="비흡연자는 0을 입력해주세요"
                    value={smokeStr}
                    onChange={(e) => setSmokeStr(digitsOnly(e.target.value).slice(0, 3))}
                ...
                <div>
                  ...
                  <input
                    ...
                    inputMode="numeric"
                    autoComplete="off"
                    placeholder="음주하지 않는 경우 0을 입력해주세요"
                    value={bingeStr}
                    onChange={(e) => setBingeStr(digitsOnly(e.target.value).slice(0, 3))}
                    className={inputClassName}
...
export default InspectionFemaleStep3;
